...
const ExpenseForm = ({ expense, onClose, onSuccess }) => {
  ...
  const { addNotification, createExpenseNotification } = useNotifications();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const expenseData = {
        ...
        createdAt: expense ? undefined : serverTimestamp(),
      ...
      if (expense) {
        ...
        await updateDoc(
          doc(db, "users", currentUser.uid, "expenses", expense.id),
          expenseData
        );
        ...
        const notification = createExpenseNotification(expenseData);
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex space-x-3 pt-4">
              ...
              <motion.button
                ...
                type="submit"
                disabled={isLoading}
                className="flex-1 bg-gradient-to-r from-blue-500 to-indigo-600 text-white py-2 rounded-lg font-medium hover:from-blue-600 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
              >
